const { isYearlyPlan, getBasePlan,
        ...
        log, error, random, forceCryptPadUpdate,
...
commands.UPDATE_QUOTA = (ctx, json, cb) => {
    const { publicKey, domain } = json;
    log(['INFO', 'updateQuota', publicKey]);
    if (!publicKey || !domain) {
        error(['EAUTH', 'updateQuota']);
        return void cb('EAUTH');
    }
    ...
    ctx.knex('subscription').whereIn('status', ENABLED_STATUS).andWhere({
        ...
        if (!arr.length) { return void cb('EFORBIDDEN'); }
        forceCryptPadUpdate(cb);
    }).catch((err) => {
        cb('EUPDATE');
    });
};
